...
def load_data(data_path='data/custom',minmax_scale_custom_data=False,task=None):
    """
...
def DEC_simple_run(x,y=None,weight_load_dir:str=None,save_dir:str='DEC_weights',minmax_scale_custom_data:bool=False,
    ...
    # setting parameters
    if initialization=='VarianceScaling':
        init = VarianceScaling(scale=1. / 3., mode='fan_in',distribution='uniform')  # [-limit, limit], limit=sqrt(1./fan_in)
    else:
        init = initialization # glorot_uniform, etc.
    
    dec = DEC(dims=[x.shape[-1]]+architecture, n_clusters=n_clusters, init=init)
    
    if weight_load_dir is None:
        dec.pretrain(x=x, y=y, optimizer=pretrain_optimizer,
                     epochs=pretrain_epochs, batch_size=batch_size,
                     save_dir=save_dir)
    else:
        dec.autoencoder.load_weights(weight_load_dir)
