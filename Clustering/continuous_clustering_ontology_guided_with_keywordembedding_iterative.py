...
gensim_model_address = datapath+'FastText Models/gensim41/FastText100D-dim-scopus-update-gensim41-w5.model'
...
if highdimensional:
    vectors_main = pd.read_csv(datapath+'Corpus/Dimensions All/embeddings/doc2vec 300D dm=1 window=10 b3')
else:
    vectors_main = pd.read_csv(datapath+'Corpus/Dimensions All/embeddings/data with abstract - tech and comp scop-dim 100D dm=1 window=12')
    
concept_column = pd.read_csv(datapath+'Corpus/Dimensions All/clean/kw ontology search/mapped concepts for keywords')
concept_column.concepts = concept_column.concepts.str.split('|')
# vectors_main['concepts'] = concept_column.concepts
concept_embeddings = pd.read_csv(datapath+'Corpus/Dimensions All/embeddings/concepts node2vec 50D p1 q05 len20 average of concepts').drop('Unnamed: 0',axis=1,inplace=False)

if highdimensional:
    vectors_main['id'] = corpus_data['id']
else:
    vectors_main = vectors_main.drop_duplicates(subset=['id'])
    
corpus_data = corpus_data[corpus_data['id'].isin(all_columns['id'])]
vectors_main = vectors_main[vectors_main['id'].isin(all_columns['id'])]


vectors_main.drop('id',axis=1,inplace=True)

corpus_data = corpus_data.merge(all_columns, on='id', how='left')
# vectors_main.PY.hist(bins=60)
corpus_data['DE-n'] = corpus_data['DE-n'].str.split(';;;')
corpus_data['DE'] = corpus_data['DE-n'].progress_apply(lambda x: x[:kw_num] if len(x)>kw_num-1 else x) # cut off to 6 keywords only


# Concat data
combined_data = pd.concat([corpus_data.reset_index(drop=True),concept_column.reset_index(drop=True),vectors_main.reset_index(drop=True),concept_embeddings.reset_index(drop=True)], axis=1)
combined_data = combined_data.dropna(subset=['id','PY','abstract'])

# Clean data
ombined_data = combined_data.drop_duplicates(subset='id', keep="last")
...
sample = combined_data.sample(5000)


# Model construct
k0 = 7
model = OGC(verbose=1,k=k0,max_keyword_distance=0.55,boundary_epsilon_growth=0,
            distance_metric='cosine',cumulative=False,auto_merge=True,auto_split=True,
            merge_regression_coefficient=0.1,split_regression_coefficient=0,
            merge_regression_offset=-98000,split_regression_offset=0,
            split_auto_threshold=5.5, merge_auto_threshold=2000) 
model.v=2
...
vectors_t0 = combined_data[combined_data.PY<2006]
keywords = vectors_t0['DE'].values.tolist()
vectors_t0.drop(['FOR_initials','PY','abstract','id','FOR','DE-n','DE','id','concepts'],axis=1,inplace=True)
...
# model_backup.v=2
# vectors_t1 = vectors_main[vectors_main.PY==2006]
# keywords = vectors_t1['DE-n'].values.tolist()
# vectors_t1.drop(['FOR_initials','PY','id','FOR','DE-n','id'],axis=1,inplace=True)
# model_backup.fit_update(vectors_t1.values, 1,keywords= keywords)
...
model = copy.deepcopy(model_backup_initial)
tmps = []
logs = []
...
model.log = True
model_backup.merge_split_iters = 1

for t,year in enumerate(range(2006,2021)):
    print('processing year',t+1,year)
    model.v=3
    vectors_t1 = combined_data[combined_data.PY==year]
    keywords = vectors_t1['DE'].values.tolist()
    vectors_t1.drop(['FOR_initials','PY','abstract','id','FOR','DE-n','DE','id','concepts'],axis=1,inplace=True)
    ...
    model_backup = copy.deepcopy(model)
model_log = model.temp
model_centroids = model.centroids


classifications = model.classifications
date = datetime.today().strftime('%Y-%m-%d')
classifications.to_csv(datapath+'Corpus/Dimensions All/clean/clustering results/'+date+' results.csv',index=False)
# with open(datapath+'Corpus/Dimensions All/clean/clustering results/'+date+' model.pkl', 'wb') as outp:
#     pickle.dump(model, outp, pickle.HIGHEST_PROTOCOL)
model_log = model.temp

# =============================================================================
#%% parameter tuning -- population vote ratio extractor
# =============================================================================
model = copy.deepcopy(model_backup_initial)
years = list(range(2006,2021))
print('processing year',1,years)
model.v=0
vectors_t1 = combined_data[combined_data.PY.isin(years)]
keywords = vectors_t1['DE'].values.tolist()
vectors_t1.drop(['FOR_initials','PY','abstract','id','FOR','DE-n','DE','id','concepts'],axis=1,inplace=True)
model.fit_update(vectors_t1.values, 1, keywords)
model_backup = copy.deepcopy(model)
model_log = model.temp
...
with open(datapath+'Corpus/Scopus new/clean/kw ontology search/keyword_search_pre-index.json') as f:
    ont_index = json.load(f)

all_columns = pd.read_csv(datapath+'Corpus/Scopus new/clean/keyword pre-processed for fasttext - nov14')[['id','eid','DE']] # use the pre-processed keyword data as the main source
all_columns_ = pd.read_csv(datapath+'Corpus/Scopus new/clean/data with abstract')[['PY']] # use data_with_abstract to get the years
concept_column = pd.read_csv(datapath+'Corpus/Scopus new/clean/mapped concepts for keywords')
concept_embeddings = pd.read_csv(datapath+'Corpus/Scopus new/embeddings/concepts/node2vec/50D p1 q05 len20 average of concepts').drop('Unnamed: 0',axis=1,inplace=False)
concept_column.concepts = concept_column.concepts.str.split('|')
...
corpus_data = pd.read_csv(datapath+'Corpus/Scopus new/clean/abstract_title method_b_3')[['abstract','id']] # get the pre-processed abstracts
corpus_data.columns = ['abstract','id_n']
vectors_main = pd.read_csv(datapath+'Corpus/Scopus new/embeddings/doc2vec 100D dm=1 window=12 gensim41')#Corpus/Scopus new/embeddings/doc2vec 300D dm=1 window=10 b3 gensim41
...
date = datetime.today().strftime('%Y-%m-%d')
classifications.to_csv(datapath+'Corpus/Scopus new/clean/clustering results/ESWA/'+date+' results.csv',index=False)
scores.to_csv(datapath+'Corpus/Scopus new/clean/clustering results/ESWA/'+date+' scores.csv',index=False)
# with open(datapath+'Corpus/Scopus new/clean/clustering results/ev8/'+date+' model.pkl', 'wb') as outp:
#     pickle.dump(model, outp, pickle.HIGHEST_PROTOCOL)
#%% SCOPUS DATA -- Node2Vec
# =============================================================================
# Load data and init
# =============================================================================
clustering_subdir = 'n2v/'
kw_num = 6 # max number of keywords per doc
datapath = '/home/sahand/GoogleDrive/Data/' #Ryzen
# datapath = '/mnt/6016589416586D52/Users/z5204044/GoogleDrive/GoogleDrive/Data/' #C1314
gensim_model_address = datapath+'FastText Models/gensim41/FastText100D-dim-scopus-update-gensim41-w5.model'
...
corpus_data.columns = ['abstract','id_n']
# vectors_main = pd.read_csv(datapath+'Corpus/Scopus new/embeddings/doc2vec 100D dm=1 window=12 gensim41')#Corpus/Scopus new/embeddings/doc2vec 300D dm=1 window=10 b3 gensim41
vectors_main = pd.read_csv(datapath+'Corpus/Scopus new/embeddings/node2vec100D p2 q1 len5-w50 mc1 mt_w2v supernodes_str_name unsorted.csv',index_col=0).reset_index()#Corpus/Scopus new/embeddings/doc2vec 300D dm=1 window=10 b3 gensim41
vectors_main['id_n'] = vectors_main['index']
vectors_main.drop('index',axis=1,inplace=True)

# Concat data
combined_data = corpus_data.merge(vectors_main,on='id_n',how='left')
combined_data = pd.concat([all_columns, combined_data,concept_embeddings], axis=1)
combined_data = combined_data.dropna(subset=['id','PY','abstract'])
...
k0 = 7
model = OGC(verbose=1,k=k0,max_keyword_distance=0.55,boundary_epsilon_growth=0,distance_metric='cosine',cumulative=False) 
model.v=0
model.set_ontology_dict(ontology_table)
model.set_keyword_embedding_model(model_AI)
model.set_ontology_keyword_search_index(ont_index)
...
model_backup.merge_split_iters = 1
# model = copy.deepcopy(model_backup)
for t,year in enumerate(range(2018,2021)):
    print('processing year',t+1,year)
    model.v=0
    vectors_t1 = combined_data[combined_data.PY==year]
    keywords = vectors_t1['DE'].values.tolist()
...
classifications = model.classifications
date = datetime.today().strftime('%Y-%m-%d')
classifications.to_csv(datapath+'Corpus/Scopus new/clean/clustering results/'+clustering_subdir+''+date+' results doublecheck.csv',index=False)
with open(datapath+'Corpus/Scopus new/clean/clustering results/'+clustering_subdir+''+date+' model doublecheck.pkl', 'wb') as outp:
    pickle.dump(model, outp, pickle.HIGHEST_PROTOCOL)
#%% Recover backup model and train using new model and prev params
# Option A
model = copy.deepcopy(model_backup)
model2 = copy.deepcopy(model_backup)

# Option B
model_backup2 = copy.deepcopy(model_backup)

k0 = 7
model = OGC(verbose=1,k=k0,distance_metric='cosine',max_keyword_distance=0.55,boundary_epsilon_growth=0) 
model.v=0
model.set_ontology_dict(ontology_table)
model.set_keyword_embedding_model(model_AI)
model.set_ontology_keyword_search_index(ont_index)
...
model.event_log = model_backup.event_log.copy()
model.populations = model_backup.populations.copy()
model.temp = model_backup.temp.copy()
model.temp_all = model_backup.temp_all.copy()
model.evolution_events = model_backup.evolution_events.copy()
model.evolution_event_counter = model_backup.evolution_event_counter
model.ontology_search_index = model_backup.ontology_search_index.copy()
model.centroids = model_backup.centroids.copy()
model.centroids_history = model_backup.centroids_history.copy()
model.centroids_history_t = model_backup.centroids_history_t
model.classifications = model_backup.classifications.copy()
model.classes_t_start = model_backup.classes_t_start.copy()
model.classifications_log = model_backup.classifications_log.copy()
model.ontology_dict = model_backup.ontology_dict.copy()
model.ontology_dict_base = model_backup.ontology_dict_base.copy()
model.model_kw = copy.deepcopy(model_backup.model_kw)
model.class_radius = model_backup.class_radius.copy()
model.columns = model_backup.columns.copy()
model.columns_vector = model_backup.columns_vector.copy()
model.golbal_boundaries = model_backup.golbal_boundaries.copy()
model.ndbbox = model_backup.ndbbox.copy()
model.radius = model_backup.radius.copy()
model.ndbbox_overlapping = model_backup.ndbbox_overlapping.copy()
model.t = model_backup.t


with open(datapath+'Corpus/Scopus new/clean/clustering results/ev6/2022-05-13 model.pkl', "rb") as input_file:
   model = pickle.load(input_file)
classifications = pd.read_csv(datapath+'Corpus/Scopus new/clean/clustering results/ev6/2022-05-13 results.csv')
...
all_keywords = list(set(list(itertools.chain.from_iterable(vectors_main['DE-n'].values.tolist()))))
...
all_keywords = pd.read_csv(datapath+'Corpus/Scopus new/clean/keywords flat nov14').fillna('')
all_keywords = all_keywords['keyword'][start:start+step].values.tolist()
...
for i,vec in tqdm(enumerate(all_vecs),total=len(all_vecs)):
    d = np.array([spatial.distance.cosine(all_vecs[i],ontology_dict[o]['vector']) for o in ontology_dict])
    index = np.argmin(d)
    distance = np.min(d)
    distances[all_keywords[i]] = (list(ontology_dict.keys())[index],distance)

distances_backup = distances.copy()
distances = distances_backup.copy()
distances_clean = {}
for x in tqdm(distances):
    if x!='' and x!=np.nan:
        distances_clean[x] = distances[x]

output_address = datapath+'Corpus/Scopus new/clean/kw ontology search/'+str(start)+' keyword_search_pre-index.json'
with open(output_address, 'w') as json_file:
    json.dump(distances_clean, json_file)

def concept_map(keywords,index):
    return [index[i] for i in keywords]
def concept_map_lvl2(keywords,index):
    concepts = []
    for kw in keywords:
...
concepts = []
concepts_lvl2 = []
for i,row in tqdm(vectors_main.iterrows(),total=vectors_main.shape[0]):
    cons = list(set(concept_map(row['DE-n'],ont_index)))
    concepts.append(str(cons).replace(']','').replace('[','').replace("', '","|").replace("'",""))
    # concepts_lvl2.append(concept_map_lvl2(cons,))
pd.DataFrame(concepts,columns=['concepts']).to_csv(datapath+'Corpus/Dimensions All/clean/kw ontology search/mapped concepts for keywords',index=False)
#%% Join the indexed parts

ont_index ={}
for i in tqdm(range(4)):
    start = 14000*i
    output_address = datapath+'Corpus/Scopus new/clean/kw ontology search/x'+str(start)+' keyword_search_pre-index.json'
...
output_address = datapath+'Corpus/Scopus new/clean/kw ontology search/keyword_search_pre-index.json'
with open(output_address, 'w') as json_file:
    json.dump(ont_index, json_file)

# Check the distances
ont_values = pd.DataFrame(ont_index.values(),columns=['kw','distance'])
distance = ont_values.distance
...
list(set(classifications_b['class'].values.tolist()))
...
clf = model_backup.classifications
clf.to_csv('1963-2015-results.csv')
X=vectors_main.drop(['FOR_initials','PY','id','FOR','DE-n','id'],axis=1).values
X_trained = model_backup.classifications[model_backup.columns_vector].values
Y_trained = model_backup.classifications['class'].values
T_trained = model_backup.classifications['t'].values

X_2d = TSNE(n_components=2, n_iter=500, verbose=2).fit_transform(X_trained)
X_3d = TSNE(n_components=3, n_iter=500, verbose=2).fit_transform(X)
X_3d = TSNE(n_components=3, n_iter=500, verbose=2).fit_transform(X_trained)




tn = 10
plot_3D(list(X_3d[T_trained==tn]),list(Y_trained[T_trained==tn]),list(Y_trained[T_trained==tn]))
...
#%% Clustering benchmark __ comparison
import pandas as pd
import numpy as np
import igraph as ig
import networkx as nx
...
datapath = '/home/sahand/GoogleDrive/Data/' #Ryzen
# Prep text abstracts, ids, and vectors
corpus_data = pd.read_csv(datapath+'Corpus/Scopus new/clean/abstract_title method_b_3')[['abstract','id']] # get the pre-processed abstracts
corpus_data.columns = ['abstract','id_n']
dpy = pd.read_csv(datapath+'Corpus/Scopus new/clean/data with abstract')[['PY']] # use data_with_abstract to get the years
corpus_data['PY'] = dpy['PY']
vectors_main = pd.read_csv(datapath+'Corpus/Scopus new/embeddings/doc2vec 100D dm=1 window=12 gensim41')#Corpus/Scopus new/embeddings/doc2vec 300D dm=1 window=10 b3 gensim41
year_filter = corpus_data[(corpus_data.PY<=2010) & (corpus_data.PY>=2000)].id_n.values.tolist()
vectors_main['id_n'] = corpus_data['id_n']
vectors_main['PY'] = corpus_data['PY']
vectors_main_year= vectors_main[(vectors_main.PY<=2010) & (corpus_data.PY>=2000)]

# =============================================================================
# Baseline benchmark
# =============================================================================
vectors_main_year.drop(['id_n','PY'],axis=1,inplace=True)
...
# KMEANS
km_model = KMeans(n_clusters=7,n_init=20, init='random', random_state=16).fit(vectors_main_year.values)
km_labels = km_model.labels_

dbi.append(davies_bouldin_score(vectors_main_year.values, km_labels))
sil.append(silhouette_score(vectors_main_year.values, km_labels))
chs.append(calinski_harabasz_score(vectors_main_year.values,km_labels))

scores = pd.DataFrame({'dbi':dbi,'sil':sil,'chs':chs})
scores['years'] = [2000,2010,2020]
date = datetime.today().strftime('%Y-%m-%d')
scores.to_csv(datapath+'Corpus/Scopus new/clean/clustering results/ESWA/'+date+' kmeans scores.csv',index=False)

# DBSCAN
eps=0.000001
eps = eps+0.05
model = DBSCAN(eps=eps, min_samples=10,n_jobs=15).fit(vectors_main_year.values)
predicted_labels = model.labels_


dbi.append(davies_bouldin_score(vectors_main_year.values, predicted_labels))
sil.append(silhouette_score(vectors_main_year.values, predicted_labels))
chs.append(calinski_harabasz_score(vectors_main_year.values,predicted_labels))

scores = pd.DataFrame({'dbi':dbi,'sil':sil,'chs':chs})
scores['years'] = [2000,2010,2020]
date = datetime.today().strftime('%Y-%m-%d')
scores.to_csv(datapath+'Corpus/Scopus new/clean/clustering results/ESWA/'+date+' DBSCAN scores.csv',index=False)

# AGGLO
model = AgglomerativeClustering(n_clusters=7,linkage='ward').fit(vectors_main_year.values)
predicted_labels = model.labels_
...
scores['years'] = [2000,2010,2020]
date = datetime.today().strftime('%Y-%m-%d')
scores.to_csv(datapath+'Corpus/Scopus new/clean/clustering results/ESWA/'+date+' Agglomerative scores.csv',index=False)

# =============================================================================
# Graph citation benchmark
# =============================================================================

# Prep citations
citation_data = pd.read_csv(datapath+'Corpus/Scopus new/clean/citations with abstracts')
citation_data = citation_data[['referring_id','cited_id']].astype(int)
citation_data_e = citation_data[((citation_data["referring_id"].isin( year_filter)) & (citation_data["cited_id"].isin( year_filter)))]
nodes = set([int(x) for x in list(citation_data_e["referring_id"].values.tolist())+list(citation_data_e["cited_id"].values.tolist())])
nodes_d = {} 
nodes_dr = {}
...
citation_data_remap = citation_data_e[['referring_id','cited_id']]#.replace(nodes_dr)

# Using nx
graph_nx = nx.Graph()
for i,row in tqdm(citation_data_remap.iterrows(),total=citation_data_remap.shape[0]):
    graph_nx.add_edge(row['referring_id'],row['cited_id'])

print('Graph fully connected:',nx.is_connected(graph_nx))
print('Connected components:',nx.number_connected_components(graph_nx))
connected_components = list(nx.connected_components(graph_nx))
largest_component = max(connected_components, key=len)
graph_nx = graph_nx.subgraph(largest_component)
vectors_main_year = vectors_main_year[vectors_main_year['id_n'].isin(largest_component)]


## Using Girvan
GN_cluster = girvan_newman(graph_nx)
# clusters_gn = list(GN_cluster)
clusters_gn = []
for i in tqdm(range(7)):  # get the 10th cluster
    clusters_gn.append(next(GN_cluster))
    

clusters_gn_4 = clusters_gn[4]
clusters_gn_5 = clusters_gn[5]
clusters_gn_6 = clusters_gn[6]
clusters_gn_7 = clusters_gn[7]
clusters_gn_8 = clusters_gn[8]

labels_4 = {int(index): label for label, indices in enumerate(clusters_gn_4) for index in indices}
labels_5 = {int(index): label for label, indices in enumerate(clusters_gn_5) for index in indices}
labels_6 = {int(index): label for label, indices in enumerate(clusters_gn_6) for index in indices}
labels_7 = {int(index): label for label, indices in enumerate(clusters_gn_7) for index in indices}
labels_8 = {int(index): label for label, indices in enumerate(clusters_gn_8) for index in indices}

label_4 = vectors_main_year.id_n.replace(labels_4)
label_5 = vectors_main_year.id_n.replace(labels_5)
label_6 = vectors_main_year.id_n.replace(labels_6)
label_7 = vectors_main_year.id_n.replace(labels_7)
label_8 = vectors_main_year.id_n.replace(labels_8)

vectors_main_year.drop(['id_n','PY','label'],axis=1,inplace=True)

dbi = []
sil = []
chs = []

dbi.append(davies_bouldin_score(vectors_main_year.values, label_4))
sil.append(silhouette_score(vectors_main_year.values, label_4))
chs.append(calinski_harabasz_score(vectors_main_year.values,label_4))

dbi.append(davies_bouldin_score(vectors_main_year.values, label_5))
sil.append(silhouette_score(vectors_main_year.values, label_5))
chs.append( calinski_harabasz_score(vectors_main_year.values,label_5))

dbi.append(davies_bouldin_score(vectors_main_year.values, label_6))
sil.append(silhouette_score(vectors_main_year.values, label_6))
chs.append(calinski_harabasz_score(vectors_main_year.values,label_6))

dbi.append( davies_bouldin_score(vectors_main_year.values, label_7))
sil.append( silhouette_score(vectors_main_year.values, label_7))
chs.append(calinski_harabasz_score(vectors_main_year.values,label_7))

dbi.append( davies_bouldin_score(vectors_main_year.values, label_8))
sil.append( silhouette_score(vectors_main_year.values, label_8))
chs.append( calinski_harabasz_score(vectors_main_year.values,label_8))
print(dbi,sil,chs)

scores = pd.DataFrame({'dbi':dbi,'sil':sil,'chs':chs})
scores['n_iter'] = [4,5,6,7,8]
scores['n_cluster'] = [6,7,8,9,10]
date = datetime.today().strftime('%Y-%m-%d')
scores.to_csv(datapath+'Corpus/Scopus new/clean/clustering results/ESWA/'+date+' GN scores <2000.csv',index=False)
...
comms = community_louvain.best_partition(graph_nx)
unique_coms = np.unique(list(comms.values()))
label_louv = vectors_main_year.id_n.replace(comms)

dbi.append( davies_bouldin_score(vectors_main_year.values, label_louv))
sil.append( silhouette_score(vectors_main_year.values, label_louv))
chs.append( calinski_harabasz_score(vectors_main_year.values,label_louv))
scores = pd.DataFrame({'dbi':dbi,'sil':sil,'chs':chs})
scores['years'] = [2000,2010,2020]
date = datetime.today().strftime('%Y-%m-%d')
scores.to_csv(datapath+'Corpus/Scopus new/clean/clustering results/ESWA/'+date+' Louvain scores.csv',index=False)

# Using igraph
citation_data_remap = citation_data_remap[((citation_data_remap["referring_id"].isin( largest_component)) & (citation_data_remap["cited_id"].isin( largest_component)))]
graphi = ig.Graph()
graphi.add_vertices(len(nodes))
graphi.add_edges(citation_data_remap.values)

# calculate dendrogram
dendrogram = graphi.community_edge_betweenness()
# convert it into a flat clustering
print(dendrogram.optimal_count)
clusters = dendrogram.as_clustering(7)

# get the membership vector
membership = clusters.membership

# Using Leiden alg
comms = leidenalg.find_partition(graphi, leidenalg.ModularityVertexPartition)
flat_clusters = comms.membership
len(set(flat_clusters))
